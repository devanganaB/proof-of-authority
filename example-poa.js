...
blockchain.createTransaction(new Transaction(Participants.accounts()[1][0],Participants.accounts()[0][0],80));

console.log("-".repeat(repeat_lines))
console.log('Creating Block 2...');
let authority = authorities[Math.floor(Math.random() * Participants.nodes().length)][0];        //SELECTING AUTHORITY PERSONNEL
console.log('\nAuthority randomly chosen: ' + authority);
blockchain.generateBlock(authority);        //block added
...
blockchain.createTransaction(new Transaction(Participants.accounts()[0][0],Participants.accounts()[1][0],200));
